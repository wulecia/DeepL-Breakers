...
train_model(task, model_A, dataset_A, tokenizer_A)

# === TASK B ===
print("\n=== TRAINING TASK B (NO FEATURES) ===")
task = "B"
tokenizer_B = AutoTokenizer.from_pretrained(MODEL_NAMES[task], use_fast=True)
dataset_B, labels_B = prepare_dataset(task)
model_B = CombinedModelNoFeatures(MODEL_NAMES[task], NUM_LABELS[task]).to(device)
...
train_model(task, model_B, dataset_B, tokenizer_B)

# === TASK C ===
print("\n=== TRAINING TASK C (NO FEATURES) ===")
task = "C"
tokenizer_C = AutoTokenizer.from_pretrained(MODEL_NAMES[task], use_fast=True)
dataset_C, labels_C = prepare_dataset(task)
model_C = CombinedModelNoFeatures(MODEL_NAMES[task], NUM_LABELS[task]).to(device)
...
train_model(task, model_C, dataset_C, tokenizer_C)
